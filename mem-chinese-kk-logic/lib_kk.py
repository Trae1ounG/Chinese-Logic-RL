...
####################################################################################
COMMON_NAMES = ['小明', '小红', '小华', '小丽', '小强', '小芳', '小刚', '小美', '小杰', '小燕', 
                '小伟', '小玲', '小军', '小娜', '小勇', '小静', '小波', '小慧', '小飞', '小云',
                '小涛', '小梅', '小海', '小兰', '小峰', '小霞', '小洋', '小萍', '小川', '小月']

UNCOMMON_NAMES = ['子墨', '雨桐', '子轩', '雨欣', '子涵', '雨晴', '子豪', '雨薇', '子睿', '雨萱',
                 '子琪', '雨婷', '子龙', '雨菲', '子文', '雨珊', '子阳', '雨嫣', '子安', '雨梦',
                 '子墨', '雨桐', '子轩', '雨欣', '子涵', '雨晴', '子豪', '雨薇', '子睿', '雨萱']
...
PREFIX = ('在一个特殊的岛屿上，只居住着{knight}和{knave}。' +
          '{Knight}总是说真话，而{knave}总是说谎。')
POSTFIX = '那么谁是{knight}，谁是{knave}？'
TEMPLATES = [
    '{name}说{content}。',
    '{name}告诉你{content}。',
    '{name}说：“{content}”。',
...
class KKProblemFormatter:
  ...
  def format_problem(self, random_names=True, random_saying_template=True,
    ...
    knight_knave = {'knight': knight_knave[0],
                    'knave': knight_knave[1],
                    'a_knight': "一个" + knight_knave[0], 'a_knave': "一个" + knight_knave[1]}
    ...
    if self.problem['solution'] is None:
      ...
      for name, indicator in zip(names, self.problem['solution']):
        if indicator:
          solution_stmts.append(name + '是' + knight_knave['knight'])
        else:
          solution_stmts.append(name + '是' + knight_knave['knave'])
      solution_text = '， '.join(solution_stmts[:-1]) + '， ' + solution_stmts[-1] + '。'
...
def format_knight_knave(names, knight_knave, statement, negation=False):
  ...
  if negation:
    text = names[statement[1]] + '不是'
  else:
    text = names[statement[1]] + '是'
...
def format_statement(names, knight_knave, statement):
  ...
  if statement[0] == 'or':
    ...
    return text
  if statement[0] == '->':
    return ('如果' + format_knight_knave(names, knight_knave, statement[1]) + '，那么' +
            format_knight_knave(names, knight_knave, statement[2]))
  if statement[0] == '<=>':
    return (format_knight_knave(names, knight_knave, statement[1]) + '当且仅当' +
            format_knight_knave(names, knight_knave, statement[2]))
...
def format_chain_of_thoughts(problem, formatted_problem, tape,
    ...
    header = "让我们一步步思考，通过考虑每个人是否在说谎，以及这是否会导致矛盾。"
    steps = []
    for step in tape[:-1]:
        if step[0] == 'proposal':
            ...
            if step[1]['outcome'] == 'ok':
                text = '假设' + t_person + '是' + t_assignment + '。'
            ...
            if len(t_exhausted) == 1:
                ...
                text += '和'.join(t_exhausted)
            else:
                t_exhausted[-1] = '和' + t_exhausted[-1]
                text += '、'.join(t_exhausted)
            text += '的所有可能性，所以让我们回到并重新考虑{P' + str(step[1]['person']) + '}。'
            steps.append(text)

    if tape[-1][0] == 'success':
        footer = '这得出了一个可行的解决方案。'
    elif tape[-1][0] == 'fail':
        footer = '所有的配置都会导致矛盾。'
    else:
        raise KeyError(f'期望成功或失败，但得到{tape[-1]}')
