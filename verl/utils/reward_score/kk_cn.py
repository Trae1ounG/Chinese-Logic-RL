...
def parse_solution_text_format(solution_text: str) -> Dict[str, str]:
    ...
    for line in solution_text.split('\n'):
        ...
        if not line:
            continue
        matches = re.findall(r'\(\d+\)\s*([\u4e00-\u9fff\s]+)', line)
        if len(matches) == 1:
            match = matches[0].strip().split("是")
            if len(match) == 2:
                name, role = match
...
def parse_model_answer(answer_text: str, expected_names: list) -> Optional[Dict[str, str]]:
    ...
    knight_count = answer_text.count('武士')
    knave_count = answer_text.count('无赖')
    ...
    for name in expected_names:
        pattern = re.compile(
            rf'\b{re.escape(name)}\s*是\s*(武士|无赖)', 
            re.IGNORECASE
        )
